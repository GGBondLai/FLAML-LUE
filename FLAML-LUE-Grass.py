# -*- coding: utf-8 -*-
'''
@File    :   FLAML-LUE-Grass.py
@Time    :   2024/12/23 19:16:52
@Author  :   GGLai 
@Version :   1.0
@Desc    :   None
'''
...
for time in timescale:
    ...
    trainStation = Integration_data[~((Integration_data['station_name'] == teststation1) & (Integration_data['Date'].dt.year >= 2010) & (Integration_data['Date'].dt.year <= 2015))]
    trainStation = trainStation[~((trainStation['station_name'] == teststation2) & (trainStation['Date'].dt.year >= 2009) & (trainStation['Date'].dt.year <= 2013))].dropna()
    trainStation = trainStation[~((trainStation['station_name'] == teststation3) & (trainStation['Date'].dt.year >= 2008) & (trainStation['Date'].dt.year <= 2010))].dropna()
    
    DLG_data = Integration_data[(Integration_data['station_name'] == teststation1) & (Integration_data['Date'].dt.year >= 2010) & (Integration_data['Date'].dt.year <= 2015)]
    HBG_S01_data = Integration_data[(Integration_data['station_name'] == teststation2) & (Integration_data['Date'].dt.year >= 2009) & (Integration_data['Date'].dt.year <= 2013)]
    DXG_data = Integration_data[((Integration_data['station_name'] == teststation3) & (Integration_data['Date'].dt.year >= 2008) & (Integration_data['Date'].dt.year <= 2010))].dropna()
  
    testStation = pd.concat([DLG_data, HBG_S01_data,DXG_data]).dropna()
    for i in range(0,18): 
        ...
        DLG_testdata = testOutput[(testOutput['station_name'] == teststation1)]
        HBG_S01_testdata = testOutput[(testOutput['station_name'] == teststation2)]
        DXG_testdata = testOutput[(testOutput['station_name'] == teststation3)]
        # Save simulation results based on different sites
        DLG_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation1}_{FLAML_name[i]}.xlsx', index=False)
        HBG_S01_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation2}_{FLAML_name[i]}.xlsx', index=False)
        DXG_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation3}_{FLAML_name[i]}.xlsx', index=False)

        # Scatter chart DLG
        test1_y = DLG_testdata['TowerGPP']
        test1_x = DLG_testdata['PredGPP']
        ...
        plt.close()
        # Scatter chart HBG_S01
        test2_y = HBG_S01_testdata['TowerGPP']
        test2_x = HBG_S01_testdata['PredGPP']
        ...
        # Scatter chart DLG
        test3_y = DLG_testdata['TowerGPP']
        test3_x = DLG_testdata['PredGPP']
        test3_r2 = 1-sklearn_metric_loss_score('r2', test3_x, test3_y)
        test3_mse = sklearn_metric_loss_score('mse', test3_x, test3_y)
        test3_mae = sklearn_metric_loss_score('mae', test3_x, test3_y)
        print('test3_r2:',test3_r2)

        plt.rcParams['font.family'] = 'Times New Roman'
        fig, ax = plt.subplots()
        fig.set_figwidth(10)
        fig.set_figheight(10)
        plt.scatter(test3_x, test3_y)
        max_x = max(test3_x)
        plt.plot([-3, max_x+3], [-3, max_x+3], color='darkblue', linestyle='--', linewidth=1.5, label='y = x')

        # Fit a linear regression line with intercept set to zero
        model = LinearRegression(fit_intercept=False)
        model.fit(test3_x.values.reshape(-1, 1), test3_y.values.reshape(-1, 1))
        a = model.coef_[0][0]
        # Plot the linear regression line
        plt.plot(test3_x, a * test3_x, color='red', linewidth=1.5, label=f'y = {a:.2f}x')
        plt.axis('equal')
        ax.text(0.1, 0.95, f'r2 = {test3_r2:.4f}', fontsize=26, color='black', transform=ax.transAxes)
        ax.text(0.1, 0.9, f'mse = {test3_mse:.4f}', fontsize=26, color='black', transform=ax.transAxes)
        ax.text(0.1, 0.85, f'mae = {test3_mae:.4f}', fontsize=26, color='black', transform=ax.transAxes)
        ax.text(0.1, 0.8, f'Best model: {automl.best_estimator}', fontsize=26, color='black', transform=ax.transAxes)

        plt.title(f'GPP Comparison_{teststation3}_{FLAML_name[i]}', fontsize=26)
        plt.xlabel('PredGPP', fontsize=26)
        plt.ylabel('TowerGPP', fontsize=26)
        plt.legend(loc='lower right',fontsize=26)
        ax.tick_params(axis='x', labelsize=26)
        ax.tick_params(axis='y', labelsize=26)
        max_value = max(max(test3_x), max(test3_y))
        plt.xlim(-1, max_value+3)
        plt.ylim(-1, max_value+3)
        plt.savefig(f'../Figure/{type}Test/Scatter{name}_{teststation3}_{FLAML_name[i]}.jpg', dpi=1000)
        plt.close()