# -*- coding: utf-8 -*-
'''
@File    :   FLAML-LUE-Forest.py
@Time    :   2024/12/23 19:05:39
@Author  :   GGLai 
@Version :   1.0
@Desc    :   None
'''
...
for time in timescale:
    ...
    trainStation = Integration_data[~((Integration_data['station_name'] == teststation1) & (Integration_data['Date'].dt.year >= 2008) & (Integration_data['Date'].dt.year <= 2010))]
    ...
    trainStation = trainStation[~((trainStation['station_name'] == teststation3) & (trainStation['Date'].dt.year >= 2011) & (trainStation['Date'].dt.year <= 2013))].dropna()

    CBF_data = Integration_data[(Integration_data['station_name'] == teststation1) & (Integration_data['Date'].dt.year >= 2008) & (Integration_data['Date'].dt.year <= 2010)]
    QYF_data = Integration_data[(Integration_data['station_name'] == teststation2) & (Integration_data['Date'].dt.year >= 2008) & (Integration_data['Date'].dt.year <= 2010)]
    BNF_data = Integration_data[(Integration_data['station_name'] == teststation3) & (Integration_data['Date'].dt.year >= 2011) & (Integration_data['Date'].dt.year <= 2013)]
    
    testStation = pd.concat([CBF_data, QYF_data,BNF_data]).dropna()
    ...
    for i in range(0,18): 
        ...
        CBF_testdata = testOutput[(testOutput['station_name'] == teststation1)]
        QYF_testdata = testOutput[(testOutput['station_name'] == teststation2)]
        BNF_testdata = testOutput[(testOutput['station_name'] == teststation3)]
        # Save simulation results based on different sites
        CBF_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation1}_{FLAML_name[i]}.xlsx', index=False)
        QYF_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation2}_{FLAML_name[i]}.xlsx', index=False)
        BNF_testdata.to_excel(f'../Output/{type}Test/Pred{name}_{teststation3}_{FLAML_name[i]}.xlsx', index=False)

        # Scatter chart CBF
        test1_y = CBF_testdata['TowerGPP']
        test1_x = CBF_testdata['PredGPP']
        ...
        plt.close()
        # Scatter chart QYF
        test2_y = QYF_testdata['TowerGPP']
        test2_x = QYF_testdata['PredGPP']
        ...
        # Scatter chart BNF
        test3_y = BNF_testdata['TowerGPP']
        test3_x = BNF_testdata['PredGPP']
